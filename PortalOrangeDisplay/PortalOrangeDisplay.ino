...
const char* OTA_HOSTNAME = "portal-orangedisplay";
...
<head>
  ...
  <title>OrangeDisplay Tuning</title>
  <style>
...
<body>
  <h2>OrangeDisplay Tuning</h2>
  <div class="small">Open while connected to OrangePortal. This adjusts CYD display behavior (decode, orientation, and display FPS cap).</div>
...
bool connectWiFi() {
  ...
  drawBootScreen("Linking to OrangePortal...");
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    drawBootScreen("OrangePortal link failed. Retrying...");
    return false;
  }
